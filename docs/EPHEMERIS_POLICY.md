...
PR34 adds runtime manifest validation. When `ASTRO_REQUIRE_PINNED_EPHEMERIS=true`, or whenever production uses `ASTRO_ENGINE=swisseph`, `ASTRO_EPHEMERIS_MANIFEST_PATH` must point to a JSON manifest generated from the approved file set. The manifest must include `fingerprint` (or `combined_fingerprint`/`ephemeris_fingerprint`) and may include `files` (or `file_manifest`) entries with `name`, `size`, and `sha256`. Any fingerprint or file-list mismatch fails closed with `EPHEMERIS_MANIFEST_MISMATCH`.
