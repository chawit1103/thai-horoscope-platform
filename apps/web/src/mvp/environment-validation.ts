...
function validateAstroCalc(env:EnvironmentInput, environment:DeploymentEnvironment):ComponentHealth {
  ...
  if (engine === "swisseph") {
    ...
    const requirePinnedEphemeris = isTrue(env.ASTRO_REQUIRE_PINNED_EPHEMERIS);
    if (environment === "production" || requirePinnedEphemeris || !allowMoshier) {
    ...
    if (environment === "production") {
      ...
      if (!requirePinnedEphemeris) errors.push(issue("SWISSEPH_EPHEMERIS_PINNING_REQUIRED", "Swiss Ephemeris production mode requires pinned ephemeris validation.", ["ASTRO_REQUIRE_PINNED_EPHEMERIS"]));
    ...
    } else if (requirePinnedEphemeris) {
