...
export interface CreateCheckoutInput { userId:string; planCode:PlanCode; successUrl:string; cancelUrl:string; currentPeriodStart:string; currentPeriodEnd:string; providerCustomerId?:string; providerSubscriptionId?:string; }
export interface CheckoutSession { id:string; provider:"mock"|"http"; checkoutUrl:string; userId:string; planCode:PlanCode; providerCustomerId?:string; providerSubscriptionId?:string; createdAt:string; status:"created"; }
...
export interface PaymentProviderState { checkoutSessions:CheckoutSession[]; processedWebhookEventIds:string[]; auditLogs:PaymentAuditLogEntry[]; receiptNotifications:EmailDeliveryResult[]; providerReferences:PaymentProviderReference[]; }
...
let state:PaymentProviderState = { checkoutSessions:[], processedWebhookEventIds:[], auditLogs:[], receiptNotifications:[], providerReferences:[] };
...
export class MockPaymentProvider implements PaymentProvider {
  ...
  async createCheckoutSession(input:CreateCheckoutInput):Promise<CheckoutSession> {
    ...
    return { id, provider:this.provider, checkoutUrl:`https://payments.example.test/checkout/${id}`, userId:input.userId, planCode:input.planCode, providerCustomerId:input.providerCustomerId, providerSubscriptionId:input.providerSubscriptionId, createdAt:(this.config.now?.() ?? new Date()).toISOString(), status:"created" };
  }
...
export class HttpPaymentProvider implements PaymentProvider {
  ...
  async createCheckoutSession(input:CreateCheckoutInput):Promise<CheckoutSession> {
    ...
    const response = await fetcher(checkoutEndpoint, {
      ...
      body:JSON.stringify({ userId:input.userId, planCode:input.planCode, successUrl:input.successUrl, cancelUrl:input.cancelUrl, currentPeriodStart:input.currentPeriodStart, currentPeriodEnd:input.currentPeriodEnd, providerCustomerId:input.providerCustomerId, providerSubscriptionId:input.providerSubscriptionId }),
    });
...
export function resetMockPaymentProviderState():void { state = { checkoutSessions:[], processedWebhookEventIds:[], auditLogs:[], receiptNotifications:[], providerReferences:[] }; }
...
export async function createPaymentCheckoutSession(provider:PaymentProvider, input:CreateCheckoutInput):Promise<CheckoutSession> {
  ...
  state.checkoutSessions.push(structuredClone(checkout));
  ...
  writePaymentAudit("payment_checkout_session_created", checkout.id, new Date(checkout.createdAt), { provider:checkout.provider, planCode:checkout.planCode, providerCustomerId:checkout.providerCustomerId ?? "", providerSubscriptionId:checkout.providerSubscriptionId ?? "" });
  return checkout;
}
...
export async function processPaymentWebhook(input:{ provider:PaymentProvider; headers:Headers; rawBody:string; receiptHook?:PaymentReceiptHook }):Promise<PaymentWebhookProcessResult> {
  ...
  const idempotencyKey = `${input.provider.provider}:${event.id}`;
  if (state.processedWebhookEventIds.includes(idempotencyKey)) {
  ...
  state.processedWebhookEventIds.push(idempotencyKey);
