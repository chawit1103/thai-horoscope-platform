...
export type OnboardingField = "birthDate"|"birthTime"|"birthPlaceText"|"timezone"|"latitude"|"longitude"|"consentBirthData";
...
export function validateOnboardingFields(input:Record<string, unknown>):OnboardingValidationResult {
  ...
  const latitudeRaw = String(input.latitude ?? "").trim();
  const longitudeRaw = String(input.longitude ?? "").trim();
  ...
  if (!normalized.birthTimeUnknown && !isValidTime(normalized.birthTime)) errors.push({ field:"birthTime", message:"กรุณาระบุเวลาเกิด หรือเลือกไม่ทราบเวลาเกิด" });
  ...
  if (latitudeRaw || longitudeRaw) {
    if (!latitudeRaw || !longitudeRaw) {
      errors.push({ field:latitudeRaw ? "longitude" : "latitude", message:"กรุณาระบุ latitude และ longitude ให้ครบคู่" });
